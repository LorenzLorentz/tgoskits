...
use axvisor_guestlib::{
    ...
    user_probe::{
        UserPageInit, build_user_aspace, code_blob, install_user_aspace, read_guest_bytes,
        restore_user_aspace,
    },
};

const CASE_ID: &str = "cpu.aarch64.sctlr.uci";
const SCTLR_UCI_BIT: u64 = 1 << 26;
...
core::arch::global_asm!(
    r#"
    .section .text.axdiff_sctlr_uci_user, "ax"
    .p2align 2
    .global axdiff_sctlr_uci_user_entry
    .global axdiff_sctlr_uci_user_entry_end
axdiff_sctlr_uci_user_entry:
    mov x9, #1
    str x9, [x0, #8]
    // IC IVAU should trap from EL0 unless SCTLR_EL1.UCI explicitly allows the
...
1:
    b 1b
axdiff_sctlr_uci_user_entry_end:
"#
);

unsafe extern "C" {
    fn axdiff_sctlr_uci_user_entry();
    fn axdiff_sctlr_uci_user_entry_end();
}
...
fn prepare_user_aspace() -> Result<(ax_mm::AddrSpace, VirtAddr, VirtAddr), &'static str> {
    let code_start = axdiff_sctlr_uci_user_entry as *const ();
    let code_end = axdiff_sctlr_uci_user_entry_end as *const ();
    let code_bytes = unsafe { code_blob(code_start, code_end) };
    // Shared page layout:
    //   [8..16]  = phase marker
    //   [16..24] = probe buffer virtual address for IC IVAU
    let mut shared = [0u8; 24];
    shared[16..24].copy_from_slice(&(USER_BUFFER_START as u64).to_le_bytes());
    let probe_buffer = [0x5Au8; 64];
...
fn run_probe(enable_uci: bool) -> Result<ProbeOutcome, &'static str> {
    let (aspace, stack_top, shared_start) = prepare_user_aspace()?;
    let original = read_sctlr_el1();
    let configured = if enable_uci {
        original | SCTLR_UCI_BIT
    } else {
        original & !SCTLR_UCI_BIT
    };
    write_sctlr_el1(configured);

    // No extra payload needs to be read back here; trap-vs-complete is fully
    // captured by the return reason and the final phase marker.
...
fn main() -> ! {
    ...
    let trap_when_clear = match run_probe(false) {
        Ok(v) => v,
        Err(stage) => emit_error(stage, "clear_uci_probe_failed"),
    };
    let allow_when_set = match run_probe(true) {
        Ok(v) => v,
        Err(stage) => emit_error(stage, "set_uci_probe_failed"),
    };
