...
//! are limited to:
//! * package-name imports (`axhal` → `ax_hal`, `axalloc` → `ax_alloc`, etc.);
...
/// Concrete `Vec`-backed per-cpu container.
pub struct PerCpuVariantsImpl<T> {
    data: Vec<T>,
}

impl<T: Send + Sync + Clone> PerCpuVariantsImpl<T> {
    /// Build an empty per-cpu container; capacity matches `cpu_num()`.
    pub fn new() -> Self {
        Self {
            data: Vec::with_capacity(ax_hal::cpu_num()),
        }
    }
    ...
    pub fn new_with_value(value: T) -> Self {
        Self {
            data: vec![value; ax_hal::cpu_num()],
        }
    }
}

impl<T: Send + Sync + Clone> Default for PerCpuVariantsImpl<T> {
    fn default() -> Self {
        Self::new()
    }
}
...
impl<T: Send + Sync + Clone> PerCpuVariants<T> for PerCpuVariantsImpl<T> {
    ...
    fn get_mut(&self) -> &mut T {
        // SAFETY: pinned per-cpu slot; preemption is disabled when called
        // from BPF helpers, so no other context observes the same slot
        // through a different `&mut`.
    ...
    unsafe fn force_get_mut(&self, cpu: u32) -> &mut T {
        // SAFETY: same invariant as `get_mut` but for an explicit CPU slot
        // chosen by the caller; the caller is responsible for ensuring no
        // concurrent aliasing for that slot.
