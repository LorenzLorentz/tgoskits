...
pub use map::{BpfMap, PollSetWrapper};
pub use prog::BpfProg;
pub use transform::{EbpfKernelAuxiliary, PerCpuImpl};
...
fn bpf_to_ax_err(e: BpfError) -> AxError {
    ...
    match e {
        ...
        B::EADDRINUSE => AxError::AddressInUse,
        B::EADDRNOTAVAIL => AxError::AddressNotAvailable,
        ...
        B::ENOTSUP => AxError::Unsupported,
        _ => AxError::InvalidInput,
    }
}

fn read_bpf_attr(uattr: usize, size: u32) -> AxResult<bpf_attr> {
    if (size as usize) < core::mem::size_of::<bpf_attr>() {
        // Linux pads short user struct with zeros: replicate so older
        // userland compiled against shorter bpf_attr still works.
    }
    ...
    // user-supplied buffer is bytewise-copied into the slot. Trailing bytes
    // were zero-initialized above when `size < sizeof(bpf_attr)`.
...
/// Output a record into a perf-event ring buffer identified by `fd`.
pub fn perf_event_output(ctx: *mut c_void, fd: usize, flags: u32, data: &[u8]) -> AxResult<()> {
    crate::perf::perf_event_output(ctx, fd, flags, data)
}
