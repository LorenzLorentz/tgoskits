...
impl PerfEventOps for TracepointPerfEvent {
    fn set_bpf_prog(&mut self, bpf_prog: Arc<dyn FileLike>) -> AxResult<()> {
        ...
        // Move both `vm` and `bpf_prog` into a `Box<dyn Any + Send + Sync>`
        // payload so the `TraceEventFunc` closure can reach them via
        // downcast on every tracepoint hit. Lifetimes line up because the
        // payload is owned for the lifetime of the registration.
        struct Ctx {
        ...
        let func: Box<dyn Fn(&[u8], &(dyn Any + Send + Sync)) + Send + Sync> =
            Box::new(|entry: &[u8], data: &(dyn Any + Send + Sync)| {
                ...
                // tracepoint contract.
                let entry = unsafe {
                    core::slice::from_raw_parts_mut(entry.as_ptr() as *mut u8, entry.len())
                };
                if let Err(e) = ctx.vm.execute_program(entry) {
                    axlog::error!("tracepoint BPF program failed: {e:?}");
                }
            });
