...
**迁移后修复补充的组件与机制**（让事件能从内核带回用户态、让探针在正确指令上触发）：① **perf ringbuf 的 `mmap(perf_fd)` 通路**——基线 `PerfEvent` 未覆写 `device_mmap`，`mmap` 返 ENODEV、`write_event` 因 `phys_addr=None` 静默丢 sample、空 Drop 泄漏；补齐 `FileLike::device_mmap → PerfEventOps::device_mmap → BpfPerfEvent::do_mmap` 分配 `1+2ⁿ` 物理页 + 初始化 `perf_event_mmap_page` 头 + 用 retainer 把页锚到用户 VMA（`close(fd)` 不释放仍被映射的内存）；② **三个 `sched:*` tracepoint**（`sched_switch`/`process_fork`/`process_exit`）——`sched_switch` 触发点在 `axtask::switch_to`，经 `ax_task::SchedTracepoint` 的 `crate_interface` late-bind 暴露（axtask 不能反向依赖 starry-kernel）；③ **uprobe 端到端**——`perf_event_open(PERF_TYPE_UPROBE)` 解析目标 ELF 的 VMA base+offset → per-process 注册 uprobe → 复用 kprobe out-of-line 单步；④ **探针点 `#[inline(never)]`**——release 下单调用点函数被 inline、留下从不执行的离线符号副本，int3 落死代码、探针永不触发，故给 `syscall::sysno`/`handle_syscall`/`sys_getpid` 强制 out-of-line；⑤ **arch-independent arg-read**——`ProbeContext::arg(0)` 映射到每架构首参寄存器（rdi/x0/a0）；⑥ 跨 crate `BpfError↔AxError` 转换、`Arc<BpfProg>` 绑定 prog 指令、`&self→&mut` 脏读改 `UnsafeCell<T>`。

**应用与运行验证**：`apps/starry/ebpf/` 下 6 个 aya 三件套程序端到端验证上述机制——`syscall_count`/`profile`（kprobe+HashMap，机制④⑤）、`kret`（kretprobe）、`mytrace`（cooked tracepoint+debugfs id）、`sched_trace`（raw tracepoint `sched_switch`+ringbuf mmap，同时验证机制①②）、`upb`（uprobe，机制③）。每个由 `cargo xtask starry app qemu -t ebpf/<name>` 一键构建（bpf-linker 编字节码 + musl 静态 loader + 注入 rootfs）并跑 qemu 断言。x86_64/qemu 实测**六个全部 PASS**且计数与 workload 严格吻合：getpid×500→精确 500、kretprobe×500、openat×200→200、uprobe arg=42→145、profile 4249 样本/18 distinct、`sched_trace` 用户态经 mmap 读回 **8582 条** `sched_switch` 记录（7 个不同 next_tid）——证明"raw tracepoint → ringbuf → mmap 读回"整条新补链路闭环。
