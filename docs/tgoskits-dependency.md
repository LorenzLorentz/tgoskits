...
flowchart TB
    ...
    subgraph sg_Axvisor__["<b>Axvisor 层</b>"]
        direction TB
        axvisor["axvisor\nv0.5.0"]
        riscv64_qemu_virt_hv["riscv64-qemu-virt-hv\nv0.5.0"]
    end
    ...
    riscv64_qemu_virt_hv --> ax_config_macros
    riscv64_qemu_virt_hv --> ax_cpu
    riscv64_qemu_virt_hv --> ax_crate_interface
    riscv64_qemu_virt_hv --> ax_kspin
    riscv64_qemu_virt_hv --> ax_lazyinit
    riscv64_qemu_virt_hv --> ax_plat
    riscv64_qemu_virt_hv --> ax_riscv_plic
    riscv64_qemu_virt_hv --> axvisor_api
    ...
    class riscv64_qemu_virt_hv cat_axvisor
...
flowchart TB
    direction TB
    L17["<b>层级 17</b><br/>堆叠层（依赖更底层 crate）<br/>`arceos-affinity`、`arceos-display`、`arceos-exception`、`arceos-fs-shell`、`arceos-irq`、`arceos-memtest`、`arceos-net-echoserver`、`arceos-net-httpclient`、`arceos-net-httpserver`、`arceos-net-udpserver`、`arceos-parallel`、`arceos-priority`、`arceos-sleep`、`arceos-tls`、`arceos-wait-queue`、`arceos-yield`、`ax-helloworld`、`ax-helloworld-myplat`、`ax-httpclient`、`ax-httpserver` …共22个"]
    classDef ls17 fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px,color:#000
    class L17 ls17
    L16["<b>层级 16</b><br/>堆叠层（依赖更底层 crate）<br/>`ax-libc`、`ax-std`、`starryos`、`starryos-test`"]
    classDef ls16 fill:#eceff1,stroke:#455a64,stroke-width:2px,color:#000
    class L16 ls16
    L15["<b>层级 15</b><br/>堆叠层（依赖更底层 crate）<br/>`ax-api`、`ax-posix-api`、`starry-kernel`"]
    classDef ls15 fill:#f8bbd0,stroke:#c2185b,stroke-width:2px,color:#000
    class L15 ls15
    L14["<b>层级 14</b><br/>堆叠层（依赖更底层 crate）<br/>`ax-feat`"]
    ...
    class L13 ls13
    L12["<b>层级 12</b><br/>堆叠层（依赖更底层 crate）<br/>`ax-net-ng`"]
    classDef ls12 fill:#e1bee7,stroke:#6a1b9a,stroke-width:2px,color:#000
    class L12 ls12
    L11["<b>层级 11</b><br/>堆叠层（依赖更底层 crate）<br/>`ax-display`、`ax-fs`、`ax-fs-ng`、`ax-input`、`ax-net`"]
    classDef ls11 fill:#ffe0b2,stroke:#ef6c00,stroke-width:2px,color:#000
    class L11 ls11
    L10["<b>层级 10</b><br/>堆叠层（依赖更底层 crate）<br/>`ax-driver`"]
    classDef ls10 fill:#fff9c4,stroke:#f9a825,stroke-width:2px,color:#000
    class L10 ls10
    L9["<b>层级 9</b><br/>堆叠层（依赖更底层 crate）<br/>`ax-dma`、`ax-sync`"]
    classDef ls9 fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px,color:#000
    class L9 ls9
    L8["<b>层级 8</b><br/>堆叠层（依赖更底层 crate）<br/>`ax-ipi`、`ax-mm`、`ax-task`"]
    classDef ls8 fill:#eceff1,stroke:#455a64,stroke-width:2px,color:#000
    class L8 ls8
    L7["<b>层级 7</b><br/>堆叠层（依赖更底层 crate）<br/>`ax-hal`、`axvm`、`hello-kernel`、`irq-kernel`、`smp-kernel`"]
    classDef ls7 fill:#f8bbd0,stroke:#c2185b,stroke-width:2px,color:#000
    class L7 ls7
    L6["<b>层级 6</b><br/>堆叠层（依赖更底层 crate）<br/>`arm_vcpu`、`ax-plat-aarch64-bsta1000b`、`ax-plat-aarch64-phytium-pi`、`ax-plat-aarch64-qemu-virt`、`ax-plat-aarch64-raspi`、`axdevice`、`riscv_vcpu`、`x86_vcpu`"]
    classDef ls6 fill:#b2ebf2,stroke:#00838f,stroke-width:2px,color:#000
    class L6 ls6
    L5["<b>层级 5</b><br/>堆叠层（依赖更底层 crate）<br/>`arm_vgic`、`ax-plat-aarch64-peripherals`、`ax-plat-loongarch64-qemu-virt`、`ax-plat-riscv64-qemu-virt`、`ax-plat-x86-pc`、`axplat-dyn`、`axplat-x86-qemu-q35`、`axvcpu`、`riscv64-qemu-virt-hv`、`riscv_vplic`、`x86_vlapic`"]
    classDef ls5 fill:#ffcdd2,stroke:#c62828,stroke-width:2px,color:#000
    class L5 ls5
    L4["<b>层级 4</b><br/>堆叠层（依赖更底层 crate）<br/>`ax-alloc`、`ax-log`、`ax-plat`、`axdevice_base`、`axvisor_api`、`starry-process`、`starry-signal`"]
    classDef ls4 fill:#e1bee7,stroke:#6a1b9a,stroke-width:2px,color:#000
    class L4 ls4
    L3["<b>层级 3</b><br/>堆叠层（依赖更底层 crate）<br/>`ax-cpu`、`ax-driver-virtio`、`ax-kspin`、`axaddrspace`、`scope-local`、`test-simple`、`test-weak`、`test-weak-partial`、`tg-xtask`"]
    classDef ls3 fill:#ffe0b2,stroke:#ef6c00,stroke-width:2px,color:#000
    class L3 ls3
    L2["<b>层级 2</b><br/>堆叠层（依赖更底层 crate）<br/>`ax-config`、`ax-driver-net`、`ax-fs-devfs`、`ax-fs-ramfs`、`ax-page-table-multiarch`、`ax-percpu`、`axbuild`、`impl-simple-traits`、`impl-weak-partial`、`impl-weak-traits`"]
...
| 3 | 堆叠层 | 组件层 | `ax-kspin` | `0.3.1` | `components/kspin` |
...
| 4 | 堆叠层 | ArceOS 层 | `ax-alloc` | `0.5.0` | `os/arceos/modules/axalloc` |
| 4 | 堆叠层 | ArceOS 层 | `ax-log` | `0.5.0` | `os/arceos/modules/axlog` |
| 4 | 堆叠层 | 组件层 | `ax-plat` | `0.5.1` | `components/axplat_crates/axplat` |
| 4 | 堆叠层 | 组件层 | `axdevice_base` | `0.4.2` | `components/axdevice_base` |
| 4 | 堆叠层 | 组件层 | `axvisor_api` | `0.5.0` | `components/axvisor_api` |
| 4 | 堆叠层 | 组件层 | `starry-process` | `0.4.0` | `components/starry-process` |
| 4 | 堆叠层 | 组件层 | `starry-signal` | `0.5.0` | `components/starry-signal` |
| 5 | 堆叠层 | Axvisor 层 | `riscv64-qemu-virt-hv` | `0.5.0` | `os/axvisor/platform/riscv64-qemu-virt-hv` |
| 5 | 堆叠层 | 平台层 | `axplat-dyn` | `0.5.0` | `platform/axplat-dyn` |
| 5 | 堆叠层 | 平台层 | `axplat-x86-qemu-q35` | `0.4.0` | `platform/x86-qemu-q35` |
| 5 | 堆叠层 | 组件层 | `arm_vgic` | `0.4.2` | `components/arm_vgic` |
| 5 | 堆叠层 | 组件层 | `ax-plat-aarch64-peripherals` | `0.5.1` | `components/axplat_crates/platforms/axplat-aarch64-peripherals` |
| 5 | 堆叠层 | 组件层 | `ax-plat-loongarch64-qemu-virt` | `0.5.1` | `components/axplat_crates/platforms/axplat-loongarch64-qemu-virt` |
| 5 | 堆叠层 | 组件层 | `ax-plat-riscv64-qemu-virt` | `0.5.1` | `components/axplat_crates/platforms/axplat-riscv64-qemu-virt` |
| 5 | 堆叠层 | 组件层 | `ax-plat-x86-pc` | `0.5.1` | `components/axplat_crates/platforms/axplat-x86-pc` |
...
| 6 | 堆叠层 | 组件层 | `ax-plat-aarch64-bsta1000b` | `0.5.1` | `components/axplat_crates/platforms/axplat-aarch64-bsta1000b` |
| 6 | 堆叠层 | 组件层 | `ax-plat-aarch64-phytium-pi` | `0.5.1` | `components/axplat_crates/platforms/axplat-aarch64-phytium-pi` |
| 6 | 堆叠层 | 组件层 | `ax-plat-aarch64-qemu-virt` | `0.5.1` | `components/axplat_crates/platforms/axplat-aarch64-qemu-virt` |
| 6 | 堆叠层 | 组件层 | `ax-plat-aarch64-raspi` | `0.5.1` | `components/axplat_crates/platforms/axplat-aarch64-raspi` |
...
| 7 | 堆叠层 | ArceOS 层 | `ax-hal` | `0.5.0` | `os/arceos/modules/axhal` |
| 7 | 堆叠层 | 组件层 | `axvm` | `0.5.0` | `components/axvm` |
| 7 | 堆叠层 | 组件层 | `hello-kernel` | `0.3.0` | `components/axplat_crates/examples/hello-kernel` |
| 7 | 堆叠层 | 组件层 | `irq-kernel` | `0.3.0` | `components/axplat_crates/examples/irq-kernel` |
| 7 | 堆叠层 | 组件层 | `smp-kernel` | `0.3.0` | `components/axplat_crates/examples/smp-kernel` |
| 8 | 堆叠层 | ArceOS 层 | `ax-ipi` | `0.5.0` | `os/arceos/modules/axipi` |
| 8 | 堆叠层 | ArceOS 层 | `ax-mm` | `0.5.0` | `os/arceos/modules/axmm` |
| 8 | 堆叠层 | ArceOS 层 | `ax-task` | `0.5.0` | `os/arceos/modules/axtask` |
| 9 | 堆叠层 | ArceOS 层 | `ax-dma` | `0.5.0` | `os/arceos/modules/axdma` |
| 9 | 堆叠层 | ArceOS 层 | `ax-sync` | `0.5.0` | `os/arceos/modules/axsync` |
| 10 | 堆叠层 | ArceOS 层 | `ax-driver` | `0.5.0` | `os/arceos/modules/axdriver` |
| 11 | 堆叠层 | ArceOS 层 | `ax-display` | `0.5.0` | `os/arceos/modules/axdisplay` |
| 11 | 堆叠层 | ArceOS 层 | `ax-fs` | `0.5.0` | `os/arceos/modules/axfs` |
| 11 | 堆叠层 | ArceOS 层 | `ax-fs-ng` | `0.5.0` | `os/arceos/modules/axfs-ng` |
| 11 | 堆叠层 | ArceOS 层 | `ax-input` | `0.5.0` | `os/arceos/modules/axinput` |
| 11 | 堆叠层 | ArceOS 层 | `ax-net` | `0.5.0` | `os/arceos/modules/axnet` |
| 12 | 堆叠层 | ArceOS 层 | `ax-net-ng` | `0.5.0` | `os/arceos/modules/axnet-ng` |
| 13 | 堆叠层 | ArceOS 层 | `ax-runtime` | `0.5.0` | `os/arceos/modules/axruntime` |
| 14 | 堆叠层 | ArceOS 层 | `ax-feat` | `0.5.0` | `os/arceos/api/axfeat` |
| 15 | 堆叠层 | ArceOS 层 | `ax-api` | `0.5.0` | `os/arceos/api/arceos_api` |
| 15 | 堆叠层 | ArceOS 层 | `ax-posix-api` | `0.5.0` | `os/arceos/api/arceos_posix_api` |
| 15 | 堆叠层 | StarryOS 层 | `starry-kernel` | `0.4.0` | `os/StarryOS/kernel` |
| 16 | 堆叠层 | ArceOS 层 | `ax-libc` | `0.5.0` | `os/arceos/ulib/axlibc` |
| 16 | 堆叠层 | ArceOS 层 | `ax-std` | `0.5.0` | `os/arceos/ulib/axstd` |
| 16 | 堆叠层 | StarryOS 层 | `starryos` | `0.4.0` | `os/StarryOS/starryos` |
| 16 | 堆叠层 | 测试层 | `starryos-test` | `0.5.0` | `test-suit/starryos` |
| 17 | 堆叠层 | ArceOS 层 | `ax-helloworld` | `0.3.0` | `os/arceos/examples/helloworld` |
| 17 | 堆叠层 | ArceOS 层 | `ax-helloworld-myplat` | `0.3.0` | `os/arceos/examples/helloworld-myplat` |
| 17 | 堆叠层 | ArceOS 层 | `ax-httpclient` | `0.3.0` | `os/arceos/examples/httpclient` |
| 17 | 堆叠层 | ArceOS 层 | `ax-httpserver` | `0.3.0` | `os/arceos/examples/httpserver` |
| 17 | 堆叠层 | ArceOS 层 | `ax-shell` | `0.3.0` | `os/arceos/examples/shell` |
| 17 | 堆叠层 | Axvisor 层 | `axvisor` | `0.5.0` | `os/axvisor` |
| 17 | 堆叠层 | 测试层 | `arceos-affinity` | `0.3.0` | `test-suit/arceos/rust/task/affinity` |
| 17 | 堆叠层 | 测试层 | `arceos-display` | `0.3.0` | `test-suit/arceos/rust/display` |
| 17 | 堆叠层 | 测试层 | `arceos-exception` | `0.3.0` | `test-suit/arceos/rust/exception` |
| 17 | 堆叠层 | 测试层 | `arceos-fs-shell` | `0.3.0` | `test-suit/arceos/rust/fs/shell` |
| 17 | 堆叠层 | 测试层 | `arceos-irq` | `0.3.0` | `test-suit/arceos/rust/task/irq` |
| 17 | 堆叠层 | 测试层 | `arceos-memtest` | `0.3.0` | `test-suit/arceos/rust/memtest` |
| 17 | 堆叠层 | 测试层 | `arceos-net-echoserver` | `0.3.0` | `test-suit/arceos/rust/net/echoserver` |
| 17 | 堆叠层 | 测试层 | `arceos-net-httpclient` | `0.3.0` | `test-suit/arceos/rust/net/httpclient` |
| 17 | 堆叠层 | 测试层 | `arceos-net-httpserver` | `0.3.0` | `test-suit/arceos/rust/net/httpserver` |
| 17 | 堆叠层 | 测试层 | `arceos-net-udpserver` | `0.3.0` | `test-suit/arceos/rust/net/udpserver` |
| 17 | 堆叠层 | 测试层 | `arceos-parallel` | `0.3.0` | `test-suit/arceos/rust/task/parallel` |
| 17 | 堆叠层 | 测试层 | `arceos-priority` | `0.3.0` | `test-suit/arceos/rust/task/priority` |
| 17 | 堆叠层 | 测试层 | `arceos-sleep` | `0.3.0` | `test-suit/arceos/rust/task/sleep` |
| 17 | 堆叠层 | 测试层 | `arceos-tls` | `0.3.0` | `test-suit/arceos/rust/task/tls` |
| 17 | 堆叠层 | 测试层 | `arceos-wait-queue` | `0.3.0` | `test-suit/arceos/rust/task/wait_queue` |
| 17 | 堆叠层 | 测试层 | `arceos-yield` | `0.3.0` | `test-suit/arceos/rust/task/yield` |

### 4.2 按层紧凑

| 层级 | 数 | 成员 |
|------|-----|------|
| 0 | 32 | `aarch64_sysreg` `ax-arm-pl011` `ax-arm-pl031` `ax-cap-access` `ax-config-gen` `ax-cpumask` `ax-crate-interface` `ax-crate-interface-lite` `ax-ctor-bare-macros` `ax-driver-base` `ax-driver-pci` `ax-errno` `ax-handler-table` `ax-int-ratio` `ax-lazyinit` `ax-linked-list-r4l` `ax-memory-addr` `ax-percpu-macros` `ax-riscv-plic` `ax-timer-list` `axbacktrace` `axpoll` `axvisor_api_proc` `bwbench-client` `cargo-axplat` `deptool` `mingo` `range-alloc-arceos` `riscv-h` `rsext4` `smoltcp` `tgmath` |
...
| 2 | 10 | `ax-config` `ax-driver-net` `ax-fs-devfs` `ax-fs-ramfs` `ax-page-table-multiarch` `ax-percpu` `axbuild` `impl-simple-traits` `impl-weak-partial` `impl-weak-traits` |
| 3 | 9 | `ax-cpu` `ax-driver-virtio` `ax-kspin` `axaddrspace` `scope-local` `test-simple` `test-weak` `test-weak-partial` `tg-xtask` |
| 4 | 7 | `ax-alloc` `ax-log` `ax-plat` `axdevice_base` `axvisor_api` `starry-process` `starry-signal` |
| 5 | 11 | `arm_vgic` `ax-plat-aarch64-peripherals` `ax-plat-loongarch64-qemu-virt` `ax-plat-riscv64-qemu-virt` `ax-plat-x86-pc` `axplat-dyn` `axplat-x86-qemu-q35` `axvcpu` `riscv64-qemu-virt-hv` `riscv_vplic` `x86_vlapic` |
| 6 | 8 | `arm_vcpu` `ax-plat-aarch64-bsta1000b` `ax-plat-aarch64-phytium-pi` `ax-plat-aarch64-qemu-virt` `ax-plat-aarch64-raspi` `axdevice` `riscv_vcpu` `x86_vcpu` |
| 7 | 5 | `ax-hal` `axvm` `hello-kernel` `irq-kernel` `smp-kernel` |
| 8 | 3 | `ax-ipi` `ax-mm` `ax-task` |
| 9 | 2 | `ax-dma` `ax-sync` |
| 10 | 1 | `ax-driver` |
| 11 | 5 | `ax-display` `ax-fs` `ax-fs-ng` `ax-input` `ax-net` |
| 12 | 1 | `ax-net-ng` |
| 13 | 1 | `ax-runtime` |
| 14 | 1 | `ax-feat` |
| 15 | 3 | `ax-api` `ax-posix-api` `starry-kernel` |
| 16 | 4 | `ax-libc` `ax-std` `starryos` `starryos-test` |
| 17 | 22 | `arceos-affinity` `arceos-display` `arceos-exception` `arceos-fs-shell` `arceos-irq` `arceos-memtest` `arceos-net-echoserver` `arceos-net-httpclient` `arceos-net-httpserver` `arceos-net-udpserver` `arceos-parallel` `arceos-priority` `arceos-sleep` `arceos-tls` `arceos-wait-queue` `arceos-yield` `ax-helloworld` `ax-helloworld-myplat` `ax-httpclient` `ax-httpserver` `ax-shell` `axvisor` |
...
| `arceos-affinity` | 17 | A simple demo to test the cpu affinity of tasks u… | `ax-std` | — |
| `arceos-display` | 17 | 系统级测试与回归入口 | `ax-std` | — |
| `arceos-exception` | 17 | 系统级测试与回归入口 | `ax-std` | — |
| `arceos-fs-shell` | 17 | 系统级测试与回归入口 | `ax-crate-interface` `ax-fs-ramfs` `ax-fs-vfs` `ax-std` | — |
| `arceos-irq` | 17 | A simple demo to test the irq state of tasks unde… | `ax-std` | — |
| `arceos-memtest` | 17 | 系统级测试与回归入口 | `ax-std` | — |
| `arceos-net-echoserver` | 17 | 系统级测试与回归入口 | `ax-std` | — |
| `arceos-net-httpclient` | 17 | 系统级测试与回归入口 | `ax-std` | — |
| `arceos-net-httpserver` | 17 | Simple HTTP server. Benchmark with Apache HTTP se… | `ax-std` | — |
| `arceos-net-udpserver` | 17 | 系统级测试与回归入口 | `ax-std` | — |
| `arceos-parallel` | 17 | 系统级测试与回归入口 | `ax-std` | — |
| `arceos-priority` | 17 | 系统级测试与回归入口 | `ax-std` | — |
| `arceos-sleep` | 17 | 系统级测试与回归入口 | `ax-std` | — |
| `arceos-tls` | 17 | 系统级测试与回归入口 | `ax-std` | — |
| `arceos-wait-queue` | 17 | A simple demo to test the wait queue for tasks un… | `ax-std` | — |
| `arceos-yield` | 17 | 系统级测试与回归入口 | `ax-std` | — |
...
| `ax-alloc` | 4 | ArceOS global memory allocator | `ax-allocator` `ax-errno` `ax-kspin` `ax-memory-addr` `ax-percpu` `axbacktrace` | `ax-api` `ax-dma` `ax-driver` `ax-feat` `ax-fs-ng` `ax-hal` `ax-mm` `ax-posix-api` `ax-runtime` `axplat-dyn` `starry-kernel` |
| `ax-allocator` | 1 | Various allocator algorithms in a unified interfa… | `ax-errno` | `ax-alloc` `ax-dma` |
| `ax-api` | 15 | Public APIs and types for ArceOS modules | `ax-alloc` `ax-config` `ax-display` `ax-dma` `ax-driver` `ax-errno` `ax-feat` `ax-fs` `ax-hal` `ax-io` `ax-ipi` `ax-log` `ax-mm` `ax-net` `ax-runtime` `ax-sync` `ax-task` | `ax-std` |
...
| `ax-config-macros` | 1 | Procedural macros for converting TOML format conf… | `ax-config-gen` | `ax-config` `ax-plat-aarch64-bsta1000b` `ax-plat-aarch64-phytium-pi` `ax-plat-aarch64-qemu-virt` `ax-plat-aarch64-raspi` `ax-plat-loongarch64-qemu-virt` `ax-plat-riscv64-qemu-virt` `ax-plat-x86-pc` `axplat-dyn` `axplat-x86-qemu-q35` `irq-kernel` `riscv64-qemu-virt-hv` `smp-kernel` |
| `ax-cpu` | 3 | Privileged instruction and structure abstractions… | `ax-lazyinit` `ax-memory-addr` `ax-page-table-entry` `ax-page-table-multiarch` `ax-percpu` `axbacktrace` | `ax-hal` `ax-plat-aarch64-bsta1000b` `ax-plat-aarch64-peripherals` `ax-plat-aarch64-phytium-pi` `ax-plat-aarch64-qemu-virt` `ax-plat-aarch64-raspi` `ax-plat-loongarch64-qemu-virt` `ax-plat-riscv64-qemu-virt` `ax-plat-x86-pc` `axplat-dyn` `axplat-x86-qemu-q35` `irq-kernel` `riscv64-qemu-virt-hv` `smp-kernel` `starry-signal` |
| `ax-cpumask` | 0 | CPU mask library in Rust | — | `ax-task` `axvisor` `axvisor_api` `axvm` |
| `ax-crate-interface` | 0 | Provides a way to define an interface (trait) in … | — | `arceos-fs-shell` `ax-driver` `ax-kernel-guard` `ax-log` `ax-plat` `ax-plat-macros` `ax-runtime` `ax-task` `axvisor` `axvisor_api` `define-simple-traits` `define-weak-traits` `fxmac_rs` `impl-simple-traits` `impl-weak-partial` `impl-weak-traits` `riscv64-qemu-virt-hv` `riscv_vcpu` `test-simple` `test-weak` `test-weak-partial` `x86_vcpu` |
...
| `ax-display` | 11 | ArceOS graphics module | `ax-driver` `ax-lazyinit` `ax-sync` | `ax-api` `ax-feat` `ax-runtime` `starry-kernel` |
| `ax-dma` | 9 | ArceOS global DMA allocator | `ax-alloc` `ax-allocator` `ax-config` `ax-hal` `ax-kspin` `ax-memory-addr` `ax-mm` | `ax-api` `ax-driver` |
| `ax-driver` | 10 | ArceOS device drivers | `ax-alloc` `ax-config` `ax-crate-interface` `ax-dma` `ax-driver-base` `ax-driver-block` `ax-driver-display` `ax-driver-input` `ax-driver-net` `ax-driver-pci` `ax-driver-virtio` `ax-driver-vsock` `ax-errno` `ax-hal` `axplat-dyn` | `ax-api` `ax-display` `ax-feat` `ax-fs` `ax-fs-ng` `ax-input` `ax-net` `ax-net-ng` `ax-runtime` `starry-kernel` |
...
| `ax-driver-net` | 2 | Common traits and types for network device (NIC) … | `ax-driver-base` `fxmac_rs` | `ax-driver` `ax-driver-virtio` `axplat-dyn` |
...
| `ax-feat` | 14 | Top-level feature selection for ArceOS | `ax-alloc` `ax-config` `ax-display` `ax-driver` `ax-fs` `ax-fs-ng` `ax-hal` `ax-input` `ax-ipi` `ax-kspin` `ax-log` `ax-net` `ax-runtime` `ax-sync` `ax-task` `axbacktrace` | `ax-api` `ax-libc` `ax-posix-api` `ax-std` `starry-kernel` `starryos` `starryos-test` |
| `ax-fs` | 11 | ArceOS filesystem module | `ax-cap-access` `ax-driver` `ax-errno` `ax-fs-devfs` `ax-fs-ramfs` `ax-fs-vfs` `ax-hal` `ax-io` `ax-lazyinit` `rsext4` | `ax-api` `ax-feat` `ax-posix-api` `ax-runtime` |
| `ax-fs-devfs` | 2 | Device filesystem used by ArceOS | `ax-fs-vfs` | `ax-fs` |
| `ax-fs-ng` | 11 | ArceOS filesystem module | `ax-alloc` `ax-driver` `ax-errno` `ax-hal` `ax-io` `ax-kspin` `ax-sync` `axfs-ng-vfs` `axpoll` `scope-local` | `ax-feat` `ax-net-ng` `ax-runtime` `starry-kernel` |
...
| `ax-hal` | 7 | ArceOS hardware abstraction layer, provides unifi… | `ax-alloc` `ax-config` `ax-cpu` `ax-kernel-guard` `ax-memory-addr` `ax-page-table-multiarch` `ax-percpu` `ax-plat` `ax-plat-aarch64-qemu-virt` `ax-plat-loongarch64-qemu-virt` `ax-plat-riscv64-qemu-virt` `ax-plat-x86-pc` `axplat-dyn` | `ax-api` `ax-dma` `ax-driver` `ax-feat` `ax-fs` `ax-fs-ng` `ax-ipi` `ax-mm` `ax-net` `ax-net-ng` `ax-posix-api` `ax-runtime` `ax-task` `axvisor` `starry-kernel` |
| `ax-handler-table` | 0 | A lock-free table of event handlers | — | `ax-plat` |
| `ax-helloworld` | 17 | ArceOS 示例程序 | `ax-std` | — |
| `ax-helloworld-myplat` | 17 | ArceOS 示例程序 | `ax-plat-aarch64-bsta1000b` `ax-plat-aarch64-phytium-pi` `ax-plat-aarch64-qemu-virt` `ax-plat-aarch64-raspi` `ax-plat-loongarch64-qemu-virt` `ax-plat-riscv64-qemu-virt` `ax-plat-x86-pc` `ax-std` | — |
| `ax-httpclient` | 17 | ArceOS 示例程序 | `ax-std` | — |
| `ax-httpserver` | 17 | Simple HTTP server. Benchmark with Apache HTTP se… | `ax-std` | — |
| `ax-input` | 11 | Input device management for ArceOS | `ax-driver` `ax-lazyinit` `ax-sync` | `ax-feat` `ax-runtime` `starry-kernel` |
...
| `ax-ipi` | 8 | ArceOS IPI management module | `ax-config` `ax-hal` `ax-kspin` `ax-lazyinit` `ax-percpu` | `ax-api` `ax-feat` `ax-runtime` |
...
| `ax-kspin` | 3 | Spinlocks used for kernel space that can disable … | `ax-kernel-guard` `ax-percpu` | `ax-alloc` `ax-dma` `ax-feat` `ax-fs-ng` `ax-ipi` `ax-log` `ax-mm` `ax-plat` `ax-plat-aarch64-bsta1000b` `ax-plat-aarch64-peripherals` `ax-plat-loongarch64-qemu-virt` `ax-plat-riscv64-qemu-virt` `ax-plat-x86-pc` `ax-std` `ax-sync` `ax-task` `axplat-x86-qemu-q35` `axvisor` `riscv64-qemu-virt-hv` `starry-kernel` `starry-process` `starry-signal` |
| `ax-lazyinit` | 0 | Initialize a static value lazily. | — | `ax-cpu` `ax-display` `ax-fs` `ax-input` `ax-ipi` `ax-mm` `ax-net` `ax-plat-aarch64-peripherals` `ax-plat-loongarch64-qemu-virt` `ax-plat-riscv64-qemu-virt` `ax-plat-x86-pc` `ax-std` `ax-task` `axaddrspace` `axplat-x86-qemu-q35` `axvisor` `riscv64-qemu-virt-hv` `starry-process` |
| `ax-libc` | 16 | ArceOS user program library for C apps | `ax-errno` `ax-feat` `ax-io` `ax-posix-api` | — |
| `ax-linked-list-r4l` | 0 | Linked lists that supports arbitrary removal in c… | — | `ax-sched` |
| `ax-log` | 4 | Macros for multi-level formatted logging used by … | `ax-crate-interface` `ax-kspin` | `ax-api` `ax-feat` `ax-posix-api` `ax-runtime` `starry-kernel` |
...
| `ax-mm` | 8 | ArceOS virtual memory management module | `ax-alloc` `ax-errno` `ax-hal` `ax-kspin` `ax-lazyinit` `ax-memory-addr` `ax-memory-set` `ax-page-table-multiarch` | `ax-api` `ax-dma` `ax-runtime` `starry-kernel` |
| `ax-net` | 11 | ArceOS network module | `ax-driver` `ax-errno` `ax-hal` `ax-io` `ax-lazyinit` `ax-sync` `ax-task` `smoltcp` | `ax-api` `ax-feat` `ax-posix-api` `ax-runtime` |
| `ax-net-ng` | 12 | ArceOS network module | `ax-config` `ax-driver` `ax-errno` `ax-fs-ng` `ax-hal` `ax-io` `ax-sync` `ax-task` `axfs-ng-vfs` `axpoll` `smoltcp` | `ax-runtime` `starry-kernel` |
...
| `ax-percpu` | 2 | Define and access per-CPU data structures | `ax-kernel-guard` `ax-percpu-macros` | `arm_vcpu` `ax-alloc` `ax-cpu` `ax-hal` `ax-ipi` `ax-kspin` `ax-plat` `ax-plat-x86-pc` `ax-runtime` `ax-task` `axplat-dyn` `axplat-x86-qemu-q35` `axvcpu` `axvisor` `axvm` `scope-local` `smp-kernel` `starry-kernel` |
| `ax-percpu-macros` | 0 | Macros to define and access a per-CPU data struct… | — | `ax-percpu` |
| `ax-plat` | 4 | This crate provides a unified abstraction layer f… | `ax-crate-interface` `ax-handler-table` `ax-kspin` `ax-memory-addr` `ax-percpu` `ax-plat-macros` | `ax-hal` `ax-plat-aarch64-bsta1000b` `ax-plat-aarch64-peripherals` `ax-plat-aarch64-phytium-pi` `ax-plat-aarch64-qemu-virt` `ax-plat-aarch64-raspi` `ax-plat-loongarch64-qemu-virt` `ax-plat-riscv64-qemu-virt` `ax-plat-x86-pc` `ax-runtime` `axplat-dyn` `axplat-x86-qemu-q35` `hello-kernel` `irq-kernel` `riscv64-qemu-virt-hv` `smp-kernel` |
| `ax-plat-aarch64-bsta1000b` | 6 | Implementation of `axplat` hardware abstraction l… | `ax-config-macros` `ax-cpu` `ax-kspin` `ax-page-table-entry` `ax-plat` `ax-plat-aarch64-peripherals` | `ax-helloworld-myplat` |
| `ax-plat-aarch64-peripherals` | 5 | ARM64 common peripheral drivers with `axplat` com… | `ax-arm-pl011` `ax-arm-pl031` `ax-cpu` `ax-int-ratio` `ax-kspin` `ax-lazyinit` `ax-plat` | `ax-plat-aarch64-bsta1000b` `ax-plat-aarch64-phytium-pi` `ax-plat-aarch64-qemu-virt` `ax-plat-aarch64-raspi` |
| `ax-plat-aarch64-phytium-pi` | 6 | Implementation of `axplat` hardware abstraction l… | `ax-config-macros` `ax-cpu` `ax-page-table-entry` `ax-plat` `ax-plat-aarch64-peripherals` | `ax-helloworld-myplat` |
| `ax-plat-aarch64-qemu-virt` | 6 | Implementation of `axplat` hardware abstraction l… | `ax-config-macros` `ax-cpu` `ax-page-table-entry` `ax-plat` `ax-plat-aarch64-peripherals` | `ax-hal` `ax-helloworld-myplat` `hello-kernel` `irq-kernel` `smp-kernel` |
| `ax-plat-aarch64-raspi` | 6 | Implementation of `axplat` hardware abstraction l… | `ax-config-macros` `ax-cpu` `ax-page-table-entry` `ax-plat` `ax-plat-aarch64-peripherals` | `ax-helloworld-myplat` |
| `ax-plat-loongarch64-qemu-virt` | 5 | Implementation of `axplat` hardware abstraction l… | `ax-config-macros` `ax-cpu` `ax-kspin` `ax-lazyinit` `ax-page-table-entry` `ax-plat` | `ax-hal` `ax-helloworld-myplat` `hello-kernel` `irq-kernel` `smp-kernel` |
| `ax-plat-macros` | 1 | Procedural macros for the `axplat` crate | `ax-crate-interface` | `ax-plat` |
| `ax-plat-riscv64-qemu-virt` | 5 | Implementation of `axplat` hardware abstraction l… | `ax-config-macros` `ax-cpu` `ax-kspin` `ax-lazyinit` `ax-plat` `ax-riscv-plic` | `ax-hal` `ax-helloworld-myplat` `hello-kernel` `irq-kernel` `smp-kernel` |
| `ax-plat-x86-pc` | 5 | Implementation of `axplat` hardware abstraction l… | `ax-config-macros` `ax-cpu` `ax-int-ratio` `ax-kspin` `ax-lazyinit` `ax-percpu` `ax-plat` | `ax-hal` `ax-helloworld-myplat` `hello-kernel` `irq-kernel` `smp-kernel` |
| `ax-posix-api` | 15 | POSIX-compatible APIs for ArceOS modules | `ax-alloc` `ax-config` `ax-errno` `ax-feat` `ax-fs` `ax-hal` `ax-io` `ax-log` `ax-net` `ax-runtime` `ax-sync` `ax-task` `scope-local` | `ax-libc` |
| `ax-riscv-plic` | 0 | RISC-V platform-level interrupt controller (PLIC)… | — | `ax-plat-riscv64-qemu-virt` `riscv64-qemu-virt-hv` |
| `ax-runtime` | 13 | Runtime library of ArceOS | `ax-alloc` `ax-config` `ax-crate-interface` `ax-ctor-bare` `ax-display` `ax-driver` `ax-fs` `ax-fs-ng` `ax-hal` `ax-input` `ax-ipi` `ax-log` `ax-mm` `ax-net` `ax-net-ng` `ax-percpu` `ax-plat` `ax-task` `axbacktrace` `axklib` | `ax-api` `ax-feat` `ax-posix-api` `starry-kernel` |
| `ax-sched` | 1 | Various scheduler algorithms in a unified interfa… | `ax-linked-list-r4l` | `ax-task` |
| `ax-shell` | 17 | ArceOS 示例程序 | `ax-std` | — |
| `ax-std` | 16 | ArceOS user library with an interface similar to … | `ax-api` `ax-errno` `ax-feat` `ax-io` `ax-kspin` `ax-lazyinit` | `arceos-affinity` `arceos-display` `arceos-exception` `arceos-fs-shell` `arceos-irq` `arceos-memtest` `arceos-net-echoserver` `arceos-net-httpclient` `arceos-net-httpserver` `arceos-net-udpserver` `arceos-parallel` `arceos-priority` `arceos-sleep` `arceos-tls` `arceos-wait-queue` `arceos-yield` `ax-helloworld` `ax-helloworld-myplat` `ax-httpclient` `ax-httpserver` `ax-shell` `axvisor` |
| `ax-sync` | 9 | ArceOS synchronization primitives | `ax-kspin` `ax-task` | `ax-api` `ax-display` `ax-feat` `ax-fs-ng` `ax-input` `ax-net` `ax-net-ng` `ax-posix-api` `starry-kernel` |
| `ax-task` | 8 | ArceOS task management module | `ax-config` `ax-cpumask` `ax-crate-interface` `ax-errno` `ax-hal` `ax-kernel-guard` `ax-kspin` `ax-lazyinit` `ax-memory-addr` `ax-percpu` `ax-sched` `ax-timer-list` `axpoll` | `ax-api` `ax-feat` `ax-net` `ax-net-ng` `ax-posix-api` `ax-runtime` `ax-sync` `starry-kernel` |
...
| `axplat-dyn` | 5 | A dynamic platform module for ArceOS, providing r… | `ax-alloc` `ax-config-macros` `ax-cpu` `ax-driver-base` `ax-driver-block` `ax-driver-net` `ax-driver-virtio` `ax-errno` `ax-memory-addr` `ax-percpu` `ax-plat` `axklib` | `ax-driver` `ax-hal` `axvisor` `starryos` |
| `axplat-x86-qemu-q35` | 5 | Hardware platform implementation for x86_64 QEMU … | `ax-config-macros` `ax-cpu` `ax-int-ratio` `ax-kspin` `ax-lazyinit` `ax-percpu` `ax-plat` | `axvisor` |
...
| `axvisor` | 17 | A lightweight type-1 hypervisor based on ArceOS | `ax-config` `ax-cpumask` `ax-crate-interface` `ax-errno` `ax-hal` `ax-kernel-guard` `ax-kspin` `ax-lazyinit` `ax-memory-addr` `ax-page-table-entry` `ax-page-table-multiarch` `ax-percpu` `ax-std` `ax-timer-list` `axaddrspace` `axbuild` `axdevice` `axdevice_base` `axhvc` `axklib` `axplat-dyn` `axplat-x86-qemu-q35` `axvcpu` `axvisor_api` `axvm` `riscv64-qemu-virt-hv` `riscv_vcpu` `riscv_vplic` | — |
| `axvisor_api` | 4 | Basic API for components of the Hypervisor on Arc… | `ax-cpumask` `ax-crate-interface` `ax-memory-addr` `axaddrspace` `axvisor_api_proc` | `arm_vcpu` `arm_vgic` `axvcpu` `axvisor` `axvm` `riscv64-qemu-virt-hv` `riscv_vcpu` `riscv_vplic` `x86_vcpu` `x86_vlapic` |
...
| `hello-kernel` | 7 | 可复用基础组件 | `ax-plat` `ax-plat-aarch64-qemu-virt` `ax-plat-loongarch64-qemu-virt` `ax-plat-riscv64-qemu-virt` `ax-plat-x86-pc` | — |
...
| `irq-kernel` | 7 | 可复用基础组件 | `ax-config-macros` `ax-cpu` `ax-plat` `ax-plat-aarch64-qemu-virt` `ax-plat-loongarch64-qemu-virt` `ax-plat-riscv64-qemu-virt` `ax-plat-x86-pc` | — |
...
| `riscv64-qemu-virt-hv` | 5 | Axvisor Hypervisor 运行时 | `ax-config-macros` `ax-cpu` `ax-crate-interface` `ax-kspin` `ax-lazyinit` `ax-plat` `ax-riscv-plic` `axvisor_api` | `axvisor` |
...
| `smp-kernel` | 7 | 可复用基础组件 | `ax-config-macros` `ax-cpu` `ax-memory-addr` `ax-percpu` `ax-plat` `ax-plat-aarch64-qemu-virt` `ax-plat-loongarch64-qemu-virt` `ax-plat-riscv64-qemu-virt` `ax-plat-x86-pc` | — |
| `starry-kernel` | 15 | A Linux-compatible OS kernel built on ArceOS unik… | `ax-alloc` `ax-config` `ax-display` `ax-driver` `ax-errno` `ax-feat` `ax-fs-ng` `ax-hal` `ax-input` `ax-io` `ax-kernel-guard` `ax-kspin` `ax-log` `ax-memory-addr` `ax-memory-set` `ax-mm` `ax-net-ng` `ax-page-table-multiarch` `ax-percpu` `ax-runtime` `ax-sync` `ax-task` `axbacktrace` `axfs-ng-vfs` `axpoll` `scope-local` `starry-process` `starry-signal` `starry-vm` | `starryos` `starryos-test` |
| `starry-process` | 4 | Process management for Starry OS | `ax-kspin` `ax-lazyinit` | `starry-kernel` |
...
| `starryos` | 16 | A Linux-compatible OS kernel built on ArceOS unik… | `ax-feat` `axbuild` `axplat-dyn` `starry-kernel` | — |
| `starryos-test` | 16 | A Linux-compatible OS kernel built on ArceOS unik… | `ax-feat` `starry-kernel` | — |
...
| `proc-macro2` `1.0.106` | — | `ax-config-macros` `ax-crate-interface` `ax-ctor-bare-macros` `ax-percpu-macros` `ax-plat-macros` `axvisor_api_proc` | — |
| `proc-macro2-diagnostics` `0.10.1` | — | — | — |
| `ptr_meta_derive` `0.1.4` | — | — | — |
| `ptr_meta_derive` `0.3.1` | — | — | — |
| `quote` `1.0.45` | — | `ax-config-macros` `ax-crate-interface` `ax-ctor-bare-macros` `ax-percpu-macros` `ax-plat-macros` `axvisor` `axvisor_api_proc` | — |
| `regex-syntax` `0.8.10` | — | — | — |
| `rkyv_derive` `0.7.46` | — | — | — |
| `schemars_derive` `1.2.1` | — | — | — |
| `syn` `1.0.109` | — | — | — |
| `syn` `2.0.117` | — | `ax-config-macros` `ax-crate-interface` `ax-ctor-bare-macros` `ax-percpu-macros` `ax-plat-macros` `axvisor` `axvisor_api_proc` | — |
| `sync_wrapper` `1.0.2` | — | — | — |
| `synstructure` `0.13.2` | — | — | — |
| `version_check` `0.9.5` | — | — | — |
...
| `embedded-hal` `1.0.0` | — | — | — |
| `tock-registers` `0.10.1` | — | `arm_vgic` `ax-cpu` `ax-riscv-plic` `riscv_vcpu` `x86_vlapic` | — |
...
| `bit-set` `0.5.3` | — | — | — |
| `bit-vec` `0.6.3` | — | — | — |
| `bit_field` `0.10.3` | — | `axaddrspace` `riscv-h` `riscv_vcpu` `x86_vcpu` | — |
| `bitbybit` `1.4.0` | — | — | — |
| `bitfield-struct` `0.11.0` | — | — | — |
| `bitmap-allocator` `0.2.1` | — | `ax-allocator` | — |
| `bitmaps` `3.2.1` | — | `arm_vgic` `ax-cpumask` `ax-page-table-multiarch` `riscv_vplic` `starry-kernel` | — |
| `block-buffer` `0.10.4` | — | — | — |
| `block-buffer` `0.12.0` | — | — | — |
| `borsh` `1.6.1` | — | — | — |
| `buddy-slab-allocator` `0.4.0` | — | `ax-alloc` `ax-dma` | — |
...
| `cesu8` `1.1.0` | — | — | — |
| `cexpr` `0.6.0` | — | — | — |
| `cfg-if` `1.0.4` | — | `ax-alloc` `ax-allocator` `ax-cpu` `ax-driver` `ax-fs-ng` `ax-hal` `ax-helloworld-myplat` `ax-kernel-guard` `ax-kspin` `ax-log` `ax-net` `ax-net-ng` `ax-percpu` `ax-percpu-macros` `ax-runtime` `ax-task` `axaddrspace` `axbacktrace` `axdevice` `axfs-ng-vfs` `axvisor` `axvm` `riscv_vcpu` `smoltcp` `starry-kernel` `starry-signal` `x86_vcpu` | — |
| `cfg_aliases` `0.2.1` | — | — | — |
...
| `equivalent` `1.0.2` | — | — | — |
| `errno` `0.3.14` | — | — | — |
| `eth-intel` `0.1.1` | — | `axplat-dyn` | — |
| `euclid` `0.22.14` | — | — | — |
| `event-listener` `5.4.1` | — | `ax-net-ng` `starry-kernel` `starry-signal` | — |
| `event-listener-strategy` `0.5.4` | — | — | — |
| `extern-trait` `0.4.3` | — | `ax-task` `axvisor` `starry-kernel` `starry-signal` `starry-vm` | — |
...
| `rd-block` `0.1.1` | — | `axplat-dyn` | — |
| `rd-net` `0.1.0` | — | `axplat-dyn` | — |
| `rdif-base` `0.7.0` | — | — | — |
| `rdif-base` `0.8.0` | — | — | — |
| `rdif-block` `0.7.0` | — | — | — |
| `rdif-clk` `0.5.0` | — | `axplat-dyn` | — |
| `rdif-def` `0.2.2` | — | — | — |
| `rdif-eth` `0.2.0` | — | — | — |
...
| `rgb` `0.8.53` | — | — | — |
| `riscv` `0.14.0` | — | `riscv-h` `riscv64-qemu-virt-hv` `riscv_vcpu` | — |
...
| `riscv-pac` `0.2.0` | — | — | — |
| `riscv-types` `0.1.0` | — | — | — |
| `riscv_goldfish` `0.1.1` | — | `ax-plat-riscv64-qemu-virt` `riscv64-qemu-virt-hv` | — |
| `riscv_plic` `0.2.0` | — | — | — |
| `rk3568_clk` `0.1.0` | — | `axplat-dyn` | — |
| `rk3588-clk` `0.1.3` | — | `axplat-dyn` | — |
| `rkyv` `0.7.46` | — | — | — |
| `rlsf` `0.2.2` | — | `ax-allocator` | — |
| `rockchip-pm` `0.4.1` | — | `axplat-dyn` | — |
...
| `ryu` `1.0.23` | — | — | — |
| `same-file` `1.0.6` | — | — | — |
| `sbi-rt` `0.0.3` | — | `ax-plat-riscv64-qemu-virt` `riscv64-qemu-virt-hv` `riscv_vcpu` | — |
| `sbi-spec` `0.0.7` | — | `riscv_vcpu` | — |
| `schannel` `0.1.29` | — | — | — |
| `schemars` `1.2.1` | — | `axbuild` `axvmconfig` | — |
| `scopeguard` `1.2.0` | — | — | — |
| `sdmmc` `0.1.0` | — | `axplat-dyn` | — |
| `seahash` `4.1.0` | — | — | — |
...
| `typeid` `1.0.3` | — | — | — |
| `typenum` `1.19.0` | — | — | — |
| `uart_16550` `0.4.0` | — | `axplat-x86-qemu-q35` `riscv64-qemu-virt-hv` | — |
...
| `valuable` `0.1.1` | — | — | — |
| `vcpkg` `0.2.15` | — | — | — |
| `virtio-drivers` `0.13.0` | — | `ax-driver-pci` `ax-driver-virtio` | — |
| `volatile` `0.3.0` | — | — | — |
| `volatile` `0.4.6` | — | — | — |
| `volatile` `0.6.1` | — | — | — |
...
| `rayon` `1.11.0` | — | — | — |
| `rayon-core` `1.13.0` | — | — | — |
| `tokio` `1.51.1` | — | `axbuild` `axpoll` `axvisor` `starryos` `tg-xtask` | — |
| `tokio-macros` `2.7.0` | — | — | — |
| `tokio-rustls` `0.26.4` | — | — | — |
| `tokio-serial` `5.4.5` | — | — | — |
